...
for i in range(4):
    ...
    solid_color = np.zeros((it1.shape[0], it1.shape[1], 3), np.uint8)
    solid_color[:] = (255, 0, 0)
...
plt.savefig('data/aerialseq.png')
plt.close()



#Visualize the motion masks
video_writer = cv2.VideoWriter('data/aerialseqmask.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 15, (seq.shape[1], seq.shape[0]))
